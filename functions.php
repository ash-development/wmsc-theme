...
if(function_exists('ttg_core_active')){
	...
	require_once get_theme_file_path("/ttgcore-setup/short/short-post-card-music.php");
...
/* Add caption under author credit if featured image has it */
...
/* Hide "Featured" category from all frontend category listings.
   Backend (admin, queries, archive URL) unaffected. */

function onair2_hide_featured_category( $categories ) {
    if ( is_admin() ) {
        return $categories;
    }
    foreach ( $categories as $key => $cat ) {
        if ( isset( $cat->slug ) && 'featured' === $cat->slug ) {
            unset( $categories[ $key ] );
        }
    }
    return array_values( $categories );
}
add_filter( 'get_the_categories', 'onair2_hide_featured_category' );


/* Shrink one-line hero bylines (.qt-byline-fit) to fit their container width. */

function onair2_byline_fit_script() {
    ?>
    <script>
    (function() {
        function fitBylines() {
            document.querySelectorAll('.qt-byline-fit').forEach(function(el) {
                el.style.fontSize = '';
        ...
        window.addEventListener('resize', function() {
            clearTimeout(t);
            t = setTimeout(fitBylines, 150);
        });
        window.addEventListener('load', fitBylines);
        document.addEventListener('DOMContentLoaded', fitBylines);
    })();
    </script>
    <?php
}
add_action( 'wp_footer', 'onair2_byline_fit_script', 99 );
